...
- Tor с настроенным скрытым сервисом получает запрос от посетителя, который переходит по адресу вида xxxxxxxx.onion. В файле конфигурации torrc есть строка:
```text
